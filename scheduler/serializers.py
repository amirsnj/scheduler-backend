...
class TaskSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "title", "description", "category",
                  "priority_level", "scheduled_date", "dead_line", "is_completed", "subTasks", "updated_at", "tags"]
        

class TaskCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Task
        fields = ["title", "description", "category", "priority_level", "scheduled_date", "dead_line"]
    ...
    def validate_category(self, value):
        ...
        request = self.context.get("request")
        if request and not TaskCategory.objects.filter(title=value, user=request.user).exists():
    ...
    def validate(self, attrs):
        ...
        if scheduled_date and dead_line and scheduled_date > dead_line:
            raise serializers.ValidationError({"detail" : "Schedule date cannot be grater than deadline date."})
...
class TaskUpdateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Task
        fields = ["title", "description", "category", "priority_level", "scheduled_date", "dead_line", "is_completed"]
...
class TaggedItemSerializer(serializers.ModelSerializer):
    task_id = serializers.PrimaryKeyRelatedField(queryset=Task.objects.none(), source='task')
    ...
    def create(self, validated_data):
        ...
        if task and TaggedItem.objects.filter(tag=tag, task=task).exists():
            raise ValidationError({"detail" : "The current task has already tagged with this selected tag."})
...
class FullTaskCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Task
        fields = ['title', 'description', 'category', 'priority_level', 'scheduled_date', 'dead_line', 'tags', 'subTasks']
...
class OptimizedTaskUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'scheduled_date', 'dead_line', 'is_completed', 'tags', 'subTasks'
        ]
