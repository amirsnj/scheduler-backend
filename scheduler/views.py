...
class TaskCategoryViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        task_count=Count('task')).filter(user=user)
    ...
    def get_queryset(self):
        ...
        queryset = Task.objects.filter(user=user)\
            ...
            .prefetch_related("subTasks", 
                              Prefetch("tagged_items", queryset=TaggedItem.objects.select_related("tag")\
                                                   .filter(tag__user=user), to_attr='prefetched_tagged_items')
            )
        ...
        if self.action == "list" and not date_param:
            ...
            queryset = queryset.filter(
                ...
                Q(scheduled_date__lt=today, dead_line__gte=today, is_completed__exact=False)
            )
