...
class TestRetriveCategory:
    def test_if_authenticated_user_is_owner_returns_200(self, authentication, api_client):
        user = authentication()
    ...
    def test_if_authenticated_user_is_not_owner_returns_404(seld, authentication, api_client):
...
class TestEditCategories:
    def test_unauthenticated_editing_categories_returns_401(self, api_client):
        ...
        response = api_client.put(
            f"/api/schedule/categories/{category.id}/",
            {"title": "new_title"}
        )
    ...
    def test_authenticated_editing_categories_returns_200(self, authentication, api_client):
        user = authentication()
        category = baker.make(TaskCategory, user=user)
        new_title = "New_title"
